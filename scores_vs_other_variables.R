startTime <- Sys.time()
cat(paste0("> Rscript scores_vs_other_variables.R\n"))

#  Rscript scores_vs_other_variables.R
...
outFold <- file.path("SCORES_VS_OTHER_VARIABLES")
...
if(buildTable) {
  ...
  datasets_variables_DT <- foreach(curr_dataset = all_datasets, .combine='rbind') %dopar% {
    ...
    all_coexprDistAUC <- eval(parse(text = load(file.path(aucCoexprDistFolder,
    ...
    pipeline_geneList <- eval(parse(text = load(file.path(pipOutFolder, 
                                                          curr_dataset, 
                                                          script0_name,
    ...
    cat("... load limma DT\n")
    limmaDT <- eval(parse(text = load(file.path(pipOutFolder, 
                                                          curr_dataset, 
                                                          script1_name,
                                                          "DE_topTable.Rdata"))))
