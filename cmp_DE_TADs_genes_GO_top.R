...
# Rscript cmp_DE_TADs_genes_GO_top.R 5 
...
outFold <- file.path("CMP_TADs_GENES_GO_top", enricher_ontologyType, nTopDS, rankVar)
...
if(buildTable){
  ...
  #***** 1) topTADs_genes
  stopifnot(typeof(universe_tadGenes) == typeof(topTADs_genes))
  stopifnot(typeof(universe_tadGenes) == typeof(c5_msigdb$gene))
  stopifnot(topTADs_genes %in% universe_tadGenes)
  stopifnot( length(topTADs_genes) <= length(universe_tadGenes) )
...
#***** 2) topGenes_manyAsPercent
  stopifnot(typeof(universe_rnaGenes) == typeof(topGenes_manyAsPercent))
  stopifnot(typeof(universe_rnaGenes) == typeof(c5_msigdb$gene))
  stopifnot(topGenes_manyAsPercent %in% universe_rnaGenes)
  stopifnot(length(universe_rnaGenes) >= length(topGenes_manyAsPercent))
  ...
  stopifnot(typeof(universe_rnaGenes) == typeof(topGenes_manyAsTopTADs))
  stopifnot(typeof(universe_rnaGenes) == typeof(c5_msigdb$gene))
  stopifnot(topGenes_manyAsTopTADs %in% universe_rnaGenes)
  stopifnot(length(universe_rnaGenes) >= length(topGenes_manyAsTopTADs))
...
for(curr_ds in unique(all_ds_DT$dataset)) {
  for(curr_type in c("topTADs_genes", "topGenes_manyAsPercent", "topGenes_manyAsTopTADs")) {
    ...
    for(var_plot in barplot_vars) {
      ...
      myTit <- paste0(barplot_vars_tit[var_plot], " - ", curr_ds, " - ", curr_type)
...
#    cat("hello5\n")
    ...
    for(var_plot in barplot_vars) {
      ...
      resultDT[,paste0("rank", "_", var_plot)] <- 1:nrow(resultDT) # not use rank because of ties
          ...
          # stopifnot(nrow(resultDT) > 0)
          # resultDT$rank <- 1:nrow(resultDT) # not use rank because of ties
          # 
...
for(var_plot in barplot_vars) {
  ...
  p_all <- ggplot(all_ds_pval_DT, aes_string(x = "rank", y = var_plot, fill = "geneType", col = "geneType")) +
  ...
  p_all <- ggplot(all_ds_pval_DT, aes_string(x = "rank", y = var_plot, fill = "geneType")) +
    geom_boxplot(outlier.shape=NA) +
    #geom_jitter() +
    scale_x_discrete(name="GO rank")+
    # scale_y_continuous(name=paste0("-log10(", padjVarGO, ")"),
