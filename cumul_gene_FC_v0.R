...
# Rscript cumul_gene_FC.R [<nTopLast>]
...
outFold <- file.path(paste0("CUMUL_GENE_FC"), nTopLast)
...
fc_ref <- seq(from=0, to = 1, by = 0.01)
...
for(positionType in unique(all_ds_geneFC_DT$position)) {
  ...
  for(curr_ds in unique(all_ds_geneFC_DT$dataset)){
    ...
    outFile <- file.path(outFold, paste0(positionType, "_", curr_ds, ".", plotType))
  ...
  outFile <- file.path(outFold, paste0(positionType, "_allDS.", plotType))
  do.call(plotType, list(outFile, height=myHeight, width=myWidth))
  plot(NULL,
       xlim = c(0,1),
       # ylim = c(0,1),
  ...
  outFile <- file.path(outFold, paste0(positionType, "_allDS_botTopColors.", plotType))
  do.call(plotType, list(outFile, height=myHeight, width=myWidth))
  plot(NULL,
       xlim = c(0,1),
       # ylim = c(0,1),
...
save(all_y, file = paste0(outFold, "all_y.Rdata"))
...
outFile <- file.path(outFold, paste0("mostFC_leastFC_allDS.", plotType))
do.call(plotType, list(outFile, height=myHeight, width=myWidth))
plot(NULL,
     xlim = c(0,1),
     # ylim = c(0,1),
...
##############################################################################
outFile <- file.path(outFold, "all_y.Rdata")
