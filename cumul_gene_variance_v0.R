...
outFold <- file.path(paste0("CUMUL_GENE_VARIANCE"), paste0(toupper(exprType), "_", nTopLast))
...
var_ref <- seq(from=0, to = 1, by = 0.01)
...
outFile <- file.path(outFold, paste0("mostVar_leastVar_allDS.", plotType))
do.call(plotType, list(outFile, height=myHeight, width=myWidth))
plot(NULL,
     xlim = c(0,1),
     # ylim = c(0,1),
