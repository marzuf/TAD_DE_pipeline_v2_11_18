startTime <- Sys.time()
cat(paste0("> Rscript coexpr_dist_diffStrand_sortNoDup.R\n"))

# Rscript coexpr_dist_diffStrand_sortNoDup.R
...
# top-ranking:
# Rscript coexpr_dist_diffStrand_sortNoDup.R TCGAcrc_msi_mss # running
...
outFold <- file.path(setDir, paste0("/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_", caller), "COEXPR_DIST_DIFF_STRAND",  paste0(curr_dataset))
system(paste0("mkdir -p ", outFold))

logFile <- file.path(outFold, paste0("coexpr_dist_diffStrand_logFile.txt"))  
...
if(buildTable){
  ...
  dataset_dist_TAD_strand_DT$diffStrand <- as.numeric(!dataset_dist_TAD_strand_DT$sameStrand)
  dataset_dist_TAD_strand_DT$sameStrand <- NULL
  
  stopifnot(!is.na(dataset_dist_TAD_strand_DT$diffStrand))
...
diffStrand_sameTAD_DT <- allData_dt[allData_dt$diffStrand == 1 & allData_dt$sameTAD == 1 ,c("gene1", "gene2", "coexpr", "dist_kb")]
...
diffStrand_diffTAD_DT <- allData_dt[allData_dt$diffStrand == 1 & allData_dt$sameTAD == 0 ,c("gene1", "gene2",  "coexpr","dist_kb")]
...
sum_4_curves_DT <- rbind(rbind(sameTAD_DT, diffTAD_DT),
                         rbind(diffStrand_sameTAD_DT,diffStrand_diffTAD_DT))
...
p_coexpr <- ggplot() +
  ...
  geom_smooth(data=allData_dt[allData_dt$diffStrand==1,], aes(x=dist_kb, y=coexpr, color=curve2), 
              method=fitMeth, se=FALSE)+
  scale_color_manual(values=mycols, name="") +
  mytheme
