...
outFold <- file.path(paste0("GENE_RANGE"), toupper(exprType))
...
if(buildTable) {
  all_ds_geneVarDT <- foreach(ds_file = all_setting_files, .combine="rbind") %dopar% {
    ...
    minRange <- range(curr_exprDT)[1]
    maxRange <- range(curr_exprDT)[2]
  ...
  outFile <- file.path(outFold, "all_ds_geneRangeDT.Rdata")
